...
TreeNode=function(attributes) {
	...
	if(!attributes.nodeCode){
		attributes.nodeCode = getUniqueID();
	}
	if(attributes.nodeCode=="")
		this.id = getUniqueID();
	else
		this.id = attributes.nodeCode;
...
TreeNode.prototype={
	initEl : function(){
		...
		this['html-element']['element'].style.padding="5px";
		this['html-element']['text'].style.fontSize="14px";
		this['html-element']['text'].style.color="#5f5f5f";
		this['html-element']['text'].style.fontFamily="黑体";
...
var ylCookie = {
    ...
    get:function(key){
        ...
        var strValue="";
		var iCookieCount=arrCookie.length;
...
function firstLeafClick(_Node){
	var cs = _Node.childNodes;
	var iCsCount=cs.length;
	if(iCsCount>0)
		firstLeafClick(cs[0]);
	else
		_Node['html-element']['text'].click();
}
function initFirstClick(){	 
	var strNodeId=ylCookie.get(sys_str_curPageCookie);
	var rootNode=testtree0_tree.root;
	var cs = rootNode.childNodes;
	var iCsCount=cs.length;
	if(iCsCount>0){
		if(strNodeId!=""){
			testtree0_tree.getNodeById(strNodeId)['html-element']['text'].click();
		}else
			firstLeafClick(cs[0]);
	}
}  
		...
		if(objRule!=null){
			...
			case "bxtx":
				if(aForm.elements[i].value==""){
					...
					alert(aForm.elements[i].attributes.ruleTip.value+",\u5fc5\u987b\u586b\u5199\uff01"); //必须填写！
			...
			case "bxsz":
				if (isNaN(aForm.elements[i].value)||aForm.elements[i].value==""){
					alert(aForm.elements[i].attributes.ruleTip.value+",\u5fc5\u987b\u662f\u6570\u5b57\uff01"); //必须是数字！
					aForm.elements[i].focus();
					return false;
				}
				break;
			case "bxsj":
				...
				if(!bIs){
					alert(aForm.elements[i].attributes.ruleTip.value+",\u5fc5\u987b\u662f\u624b\u673a\uff01"); //必须是手机！
					aForm.elements[i].focus();
					return false;
				}
				break;
			...
			case "bxdh":
				...
				if(!bIs){
					alert(aForm.elements[i].attributes.ruleTip.value+",\u5fc5\u987b\u662f\u7535\u8bdd\uff01"); //必须是电话！
					aForm.elements[i].focus();
					return false;
				}
				break;
			...
			case "bxem":
				...
				if(!bIs){
					alert(aForm.elements[i].attributes.ruleTip.value+",必须是邮件！"); //必须是电子邮件！
					aForm.elements[i].focus();
					return false;
				}
				break;
			...
			case "bncf":
				if(aForm.elements[i].value==""){
					alert("\u8bf7\u586b\u5199"+aForm.elements[i].attributes.ruleTip.value); //请填写
					aForm.elements[i].focus();
					return false;
				}
				...
				if(strIsCf=="ok"){
					alert(aForm.elements[i].attributes.ruleTip.value+"\u5df2\u7ecf\u88ab\u5360\u7528\u002c\u8bf7\u91cd\u65b0\u8f93\u5165\u0021"); //已经被占用,请重新输入!
					aForm.elements[i].focus();
					return false;
				}
				break;
	...
	for(var i=0;i<iFormSize;i++){
		...
		if(iRule!=null){
			...
			case "bncf":
				if(aForm.elements[i].value==""){
					//请填写
					alert("\u8bf7\u586b\u5199"+aForm.elements[i].ruleTip);
					return false;
				}
				break;
			}
...
function getCharData(strTJFW,strTJKJ,strSJMC,strTJSJ){	
	//统计图
	var strXmlData="<chart palette='2' caption='\u7edf\u8ba1\u56fe' shownames='1' >";
	var strXmlFW="";
	var strXmlKJ="";
	...
	for(var i=0;i<i_KJ_LEN;i++){//范围是行，口径是列
...
function doPageClick(_obj,_objCom,_iIndex){
	...
	$(_objCom.id+"class_"+sysGetAttr(_objCom,'icurtarget')+"_rt").className='sys_tabs_right';//原选中页右边风格
	$(_objCom.id+"class_"+sysGetAttr(_objCom,'icurtarget')+"_lt").className='sys_tabs_left';//原选中页右边风格
	$(_objCom.id+"class_"+_iIndex+"_rt").className='sys_tabs_right_current';//原选中页右边风格
	$(_objCom.id+"class_"+_iIndex+"_lt").className='sys_tabs_left_current';//原选中页右边风格
...
function sys_synVScroll(){
 ...
 function SetGrid(awidth,aheight){
	...
	if(queryTable!=null){
		...
		if(queryTable.offsetWidth>iCurBodyWidth)//有横向滚动条
			iScroll=8;
 ...
 function synTitle(obj){
  ...
  function sys_addMod(){
	...
	if(arrCurSelRow.length>0)
		if(arrCurSelRow.length>1)
			alert("\u6700\u591a\u53ea\u80fd\u9009\u4e2d\u4e00\u4e2a\u6a21\u5757\uff01"); //最多只能选中一个模块！
		else{
			miniWin('\u589e\u52a0\u6a21\u5757','','ModMain?STROPTYPE=1&SMODCODE='+arrCurSelRow[0].cells[2].innerText,700,200,'',''); //增加模块
			}
	else
		miniWin('\u589e\u52a0\u6a21\u5757','','ModMain?STROPTYPE=1&SMODCODE=',700,200,'',''); //增加模块
  } 
  ...
  function sys_EditForm(_iWidth,_iHeight){
	...
	if(arrCurSelRow.length>0)
		if(arrCurSelRow.length>1)
			alert("\u6700\u591a\u53ea\u80fd\u9009\u4e2d\u4e00\u6761\u8bb0\u5f55\u8fdb\u884c\u4fee\u6539\uff01"); //最多只能选中一条记录进行修改！
		else{
			var strUrl=$("syseditvalue"+arrCurSelRow[0].value).value;
			miniWin('\u4fee\u6539\u4fe1\u606f','',strUrl,_iWidth,_iHeight,'',''); //修改信息
			}
	else
		alert("\u8bf7\u9009\u62e9\u4e00\u6761\u4fee\u6539\u9879\uff01"); //请选择一条修改项！
  }
function sys_CopyForm(_iWidth,_iHeight){
    var arrCurSelRow=sys_getCurSelCheckBox();
	if(arrCurSelRow.length>0)
		if(arrCurSelRow.length>1)
			alert("\u6700\u591a\u53ea\u80fd\u9009\u4e2d\u4e00\u6761\u8bb0\u5f55\u8fdb\u884c\u590d\u5236\uff01"); //最多只能选中一条记录进行复制！
		else{
			var strUrl=$("syseditvalue"+arrCurSelRow[0].value).value+"&sys_copy=true";
			miniWin('\u590d\u5236\u4fe1\u606f','',strUrl,_iWidth,_iHeight,'',''); //复制信息
			}
	else
		alert("\u8bf7\u9009\u62e9\u4e00\u6761\u590d\u5236\u9879\uff01"); //请选择一条复制项！
  }  
...
function sys_AddCondition(_obj){
	...
	if(strCurSelValue!=')'){
		...
		if(iCurRowIndex==iRowCount){
			...
			sys_generCell(objNewRow,"kh"+sys_iAddConRowIndex,add.sys_kuohaodata);//括号	
			sys_generCell(objNewRow,"zd"+sys_iAddConRowIndex,add.sys_fielddata);//字段
			sys_generCell(objNewRow,"tj"+sys_iAddConRowIndex,add.sys_tiaojiandata);//条件
			...
			"' onclick='sys_doConditionSelValue(this);' style='width:160px;'><input id='tjzdate"+sys_iAddConRowIndex+"'  type='hidden'>";//条件值ֵ
			sys_generCell(objNewRow,"ljf"+sys_iAddConRowIndex,add.sys_luojifudata);//逻辑符
			objNewCell=objNewRow.insertCell();
			objNewCell.className='td1';
...
function sys_generCell(_objNewRow,_strName,_strValueSrc){
	...
	objNewCell.className='td1';
	objNewCell.innerHTML=_strValueSrc.value+"id='"+_strName+"' name='"+_strName+"'>";//括号
...
function $_sys_sel_tip(){
	...
	//计数
	var result="\u8ba1\u6570:&nbsp;&nbsp;<font color='blue'><b>"+(i_sys_msd_end_row-i_sys_msd_start_row+1)+"</b></font>";
	var fTotal=0;
	var iIndex=0;
	...
	//求和 平均值
	if(fTotal!=0)
		result+="&nbsp;&nbsp;\u6c42\u548c:&nbsp;&nbsp;<font color='blue'><b>"+fTotal+"</b></font>&nbsp;&nbsp;\u5e73\u5747\u503c:&nbsp;&nbsp;<font color='blue'><b>"+(fTotal/iIndex).toFixed(2)+"</b></font>";
...
function sys_open_file(_obj,_strUrl){
	//文档查看
	miniWinAll('\u6587\u6863\u67e5\u770b','',_strUrl,parent.parent.parent.iScreen_Width,parent.parent.parent.iScreen_Height-60,'','',0,60,0);
}
 ...
 * @returns
 */
workTicketExport = function (pagecode) {
	var arrCurSelRow=sys_getCurSelCheckBox();
    if(arrCurSelRow.length>0){
        if(arrCurSelRow.length>1){
        	//最多只能选中一条记录进行导出！
            alert("\u6700\u591a\u53ea\u80fd\u9009\u4e2d\u4e00\u6761\u8bb0\u5f55\u8fdb\u884c\u5bfc\u51fa\uff01");
        }else{
            var strComponents = arrCurSelRow[0].value;
            var arr = sFlow_evet.S_RUN_AGR[strComponents].innerHTML.split(",");
            
            try{
            	var result=getTx("S_ID="+arr[0]+"&PAGECODE="+pagecode,"work_ticket_export.v");
                var resultMsg = subGetTx(result,"workTicketExportResult");
                ...
                console.log("err:015->workTicketExport");
            }
        }
    }else{
    	//请选择一条导出项 

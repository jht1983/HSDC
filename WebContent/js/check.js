		...
		if(objRule!=null){
			...
			case "bxtx":
				if(aForm.elements[i].value==""){
					...
					alert(aForm.elements[i].attributes.ruleTip.value+",必须填写！");
			...
			case "bxsz":
				if (isNaN(aForm.elements[i].value)||aForm.elements[i].value==""){
					alert(aForm.elements[i].attributes.ruleTip.value+",必须是数字！");
					aForm.elements[i].focus();
					return false;
				}
				break;
			case "bxsj":
				...
				if(!bIs){
					alert(aForm.elements[i].attributes.ruleTip.value+",必须是手机！");
					aForm.elements[i].focus();
					return false;
				}
				break;
			...
			case "bxdh":
				...
				if(!bIs){
					alert(aForm.elements[i].attributes.ruleTip.value+",必须是电话！");
					aForm.elements[i].focus();
					return false;
				}
				break;
			...
			case "bxem":
				...
				if(!bIs){
					alert(aForm.elements[i].attributes.ruleTip.value+",必须是电子邮件！");
					aForm.elements[i].focus();
					return false;
				}
				break;
			...
			case "bncf":
				if(aForm.elements[i].value==""){
					alert("请填写"+aForm.elements[i].attributes.ruleTip.value);
					aForm.elements[i].focus();
					return false;
				}
				...
				if(strIsCf=="ok"){
					alert(aForm.elements[i].attributes.ruleTip.value+"已经被占用,请重新输入!");
					aForm.elements[i].focus();
					return false;
				}
				break;
  ...
  function sys_addMod(){
	...
	if(arrCurSelRow.length>0)
		if(arrCurSelRow.length>1)
			...
			miniWin('增加模块','','ModMain?STROPTYPE=1&SMODCODE='+arrCurSelRow[0].cells[2].innerText,700,200,'','');
			}
	else
		miniWin('增加模块','','ModMain?STROPTYPE=1&SMODCODE=',700,200,'','');
  } 
...
function sys_CopyForm(_iWidth,_iHeight){
	...
	if(arrCurSelRow.length>0)
		if(arrCurSelRow.length>1)
			...
			miniWin('复制信息','',strUrl,_iWidth,_iHeight,'','');
			}
	else
		alert("请选择一条复制项！");
  }  
...
function sys_AddCondition(_obj){
	...
	if(strCurSelValue!=')'){
		...
		if(iCurRowIndex==iRowCount){
			...
			"' onclick='sys_doConditionSelValue(this);' style='width:160px;'><input id='tjzdate"+sys_iAddConRowIndex+"'  type='hidden'>";//条件值

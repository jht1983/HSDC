...
// 	              htpSess.setAttribute("SYS_STRBRANCHNAME",objBranchName);
  ...
  public StringBuffer getUserInfo(){
   ...
   ÊµÅÁ®ãÊåÇËµ∑
	 ...
	 * ÊåÇËµ∑ÊµÅÁ®ã
	 * @param _strFlowId ÊµÅÁ®ãÂè?
	 * @param _strFlowRunId ËäÇÁÇπÂè?
	 * @param _strVersion ÁâàÊú¨Âè?
	 * @param _strType 1:ÊåÇËµ∑ 2:ÂêØÁî®
	 * @return
   ...
   */ 
    ...
    public StringBuffer htmlToMethod(){
        ...
        boolean bol = false;
        if (disType == null && disType.trim().length() > 0) {// Ë°®ÂçïÂà§Á©∫
        ...
        //             String sd = sdf.format(new Date(timeStamp)); 
        			...
        			//”‹…Ò»»µÁXX∞‡π§◊˜»’÷æ+»’∆⁄
        			String fileName = "Ê¶ÜÁ•ûÁÉ≠Áîµ"+bzmc+"Â∑•‰ΩúÊó•Âøó"+rq+".docx";
	...
	public StringBuffer getFormFields() {
		...
		if (formCod == null && formCod.trim().length() > 0) {// Ë°®ÂçïÂà§Á©∫
			sb.append("ÊµÅÁ®ãID‰∏∫Á©∫");
			return sb;
		}
		if ("2".equals(fromType)) {// Âçï‰∏ÄË°®Âçï
    ...
    public void MeasuresToolEntr(HttpServletRequest request){
		...
		case "1505896107531":
			...
			break;
		case "1516247158225"://Eric‰øÆÊîπ 2018Âπ?2Êú?5Êó? 15:27:28
  
        
			sc.checkToolBusinessDeal(S_ID);
			
			break;
		case "1516166904515"://Eric‰øÆÊîπ 2018Âπ?2Êú?5Êó? 15:27:39
            
          
    		sc.branchSafeCheck(S_ID);
		
	    	break;
	    case "15175538437610"://Eric‰øÆÊîπ 2018Âπ?2Êú?23Êó? 10:09:59
            
          
    		sc.affirmSafeCheck(S_ID);
		
	    	break;	
	    case "1516587886146"://Eric‰øÆÊîπ 2018Âπ?2Êú?23Êó? 17:11:33
            
          
    		sc.modifySafeCheck(S_ID);
		
	    	break;
    	case "1516602563575"://Eric‰øÆÊîπ 2018Âπ?2Êú?24Êó? 15:13:12
            
          
    		sc.alterSafeCheck(S_ID);
		
	    	break;
	    case "1516606174518"://Eric‰øÆÊîπ 2018Âπ?2Êú?24Êó? 16:19:31
            
          
    		sc.ratHazard(S_ID);
		
	    	break;
	    case "1516613463357"://Eric‰øÆÊîπ 2018Âπ?2Êú?24Êó? 16:48:46
	    ...
	    case "1506310525794"://Eric‰øÆÊîπ 2018Âπ?2Êú?28Êó? 11:43:54
	      ...
	      case "1515723789958"://Eric‰øÆÊîπ 2018Âπ?3Êú?5Êó? 13:17:55
            
          
    		sc.borrowTools(S_ID);
		
	    	break;
	    case "1522727526758"://Eric 2018ƒÍ4‘¬4»’ 13:52:53//ºº ı
	    ...
	    case "1522732741869"://Eric 2018-4-4 13:54:22 ”≈–„
	    	...
	    	break;
	   case "1522719345443"://Eric 2018-4-4 13:54:37 ∫œ¿ÌªØΩ®“È
     ...
     * Mantra
     * 
     * ÂêëEventClÊ∑ªÂä†ÂêëÈáèÂÄ?
     * 
     **/
    public void timingTaskData() throws Exception{   // Áî±‰∫éËøîÂõûÁöÑÊï∞Êç?  Êò?  <String,String[]> Áö? Êâ?‰ª? Èú?Ë¶ÅÈÅçÂé?
        HashMap<String, String> vecAddDate = new HashMap<String, String>();  //ËøîÂõûÁöÑHashMap
        String _strSplitN=",";  //Â¶ÇÊûúÊï∞ÊçÆÈáçÂ§ç  Â∑≤È?óÂè∑ÂàÜÈöî
        Map hm = request.getParameterMap();  //Ëé∑Âèñ‰º†ËøõÊù•ÁöÑÊâ?ÊúâÂèÇÊï?
        Iterator entries = hm.entrySet().iterator(); //ÂàõÂª∫Ëø≠‰ª£Âô?
        while (entries.hasNext()) {  
            Map.Entry entry = (Map.Entry) entries.next();  //ÂæóÂà∞ÂΩìÂâçÁö? KEYVALUE
            String sys_PageKey = (String) entry.getKey();  
            if(sys_PageKey.indexOf("NO_")==0){ //ËøáÊª§NOÂº?Â§¥ÁöÑ
                continue;
            }
            if(sys_PageKey.indexOf("$")==0){ //ËøáÊª§$Âº?Â§¥ÁöÑ
                continue;
            }
            ...
            for(String a : (String [])entry.getValue()){  //Â¶ÇÊûúÊï∞ÊçÆÈáçÂ§ç ÊãºÊé•
                sys_PageValue.append(new String(a.getBytes("iso8859-1"),"GBK"));
                sys_PageValue.append(_strSplitN);
            }
            vecAddDate.put(sys_PageKey, sys_PageValue.substring(0,sys_PageValue.length()-_strSplitN.length()));   // ÂêëËøîÂõûÁöÑHashMap Ê∑ªÂä†KEY VALUE ;
        }
...
public StringBuffer backFlow(){
    ...
    public StringBuffer tableLatestData()
      {
        ...
        if ((tableNameToSelect != null) && (tableNameToSelect.trim().length() > 0)) {
            ...
            localStringBuffer.append("/**********Ë°®Âêç‰∏çËÉΩ‰∏∫Á©∫********/");
            return localStringBuffer;
        }
        if ((tableCol != null) && (tableCol.trim().length() > 0)) {
            
        }else{
            localStringBuffer.append("/**********Âàó‰∏çËÉΩ‰∏∫*ÊàñË?ÖÁ©∫********/");
            return localStringBuffer;
        }
  ...
  public StringBuffer TheGateway() {
    StringBuffer localStringBuffer = new StringBuffer();
    localStringBuffer.append("<table id='flowgate' width='100%' class='table1'><col width='30%'/><col width='10%'/><col /><tr><th class='td1'>ËäÇÁÇπÂêçÁß∞ </th><th class='td1'>ÈªòËÆ§ </th><th class='td1'>Êù°‰ª∂ËÆæÁΩÆ | ÊòØÂê¶ÊâãÂä®ÈÄâÊã©ËäÇÁÇπ<input id='SFSDXZJD' type='checkbox' style='vertical-align: middle;' onclick='morenxuanze(this)' /> </th></tr");
    localStringBuffer.append("</table>");
    localStringBuffer.append("<table class='table1' cellpadding='0' cellspacing='0' width='100%' style='display:black;position:fixed; bottom:0;'><tbody><tr><td colspan='2' align='right' class='bttformoparea'><table><tbody><tr><td><a id='submitToObj' class='button green'><span><div class='bttn_panel' style='background-image:url(images/eve/qd.png);'>Á°ÆÂÆö</div></span></a></td><td width='50px'>&#160;</td></tr></tbody></table></td></tr></tbody></table>");
   ...
   public StringBuffer TheGateway2() {
    ...
    localStringBuffer.append("<td class='td1'>Âà?</td>");
    localStringBuffer.append("<td class='td1'>Êù°‰ª∂</td>");
    localStringBuffer.append("<td class='td1'>Êù°‰ª∂</td>");
    localStringBuffer.append("<td class='td1'>ÊãºÊé•</td>");
    localStringBuffer.append("<td class='td1'></td>");
    localStringBuffer.append("</tr>");
    localStringBuffer.append("</table>");
...
try {
    ...
    public StringBuffer tempSelectfs(String _strSPAGECODE, String _strSplit) {
        ...
        returnStringBuffer.append("\\$");
        for (int z = 0; z < iArray; z++) {//arrayOfString[1][j]‰∏≠Êñá //arrayOfString[0][j]‰ª£Á¢º  Ë°?$Âà?
        ...
        try {
            ...
            if ((str1 != null) && (str1.trim().length() > 0)) {
                ...
                for (int i = 0; i < iRecordCount; i++) {
                    ...
                    String SFIELDCODE = record.getFieldByName("SFIELDCODE").value.toString();
                    if("9".equals(SPAGETYPE)){
                        returnValue.append("<table id='"+SFIELDCODE+"#tableCol1' class='tableCol' cellpadding='0' cellspacing='0' width='50%' style=\"float: left;table-layout:fixed;display:none;\"><col width='' /><col width='9%' /><col width='9%' /><col width='9%' /><col width='25%' /><tr class='tr1'><td class='inputth1'>Â≠óÊÆµÂêçÁß∞ </td><td class='inputth1' style='display:none;'>Â≠óÊÆµ‰ª£Á†Å </td><td class='inputth1'>Êü•Áúã </td><td class='inputth1'>ÁºñËæë </td><td class='inputth1'>ÂøÖÂ°´ </td><td class='inputth1'>Ëß¶Âèë‰∫ãÂä°</td></tr>");
  ...
  public StringBuffer createDataBase() {
        ...
        StringBuffer SFIELDS = new StringBuffer(); //ÊòæÁ§∫ÁöÑÂ≠óÊÆ?   ,ÂÖàÂÖ®‰∏∫Ëã±Êñ?  Âº?Â§¥Êúâ,
        StringBuffer SFIELDSQLS = new StringBuffer();// Êü•ËØ¢ÁöÑÊâÄÊú?   Âº?Â§¥Êúâ,
        StringBuffer SFIELDNAMES = new StringBuffer();//‰∏≠ÊñáÂêçÁß∞  Âº?Â§¥Êúâ,
        String STABLES = "";//Ë°®‰ª£Á†?
        String SCONNAME = "";//SCONNAME_MEANING = SCONNAME  ‰∏∫Ë°®Âê?
        String SCONID = Long.toString(timeL); //ÊµÅÊ∞¥Âè?
        StringBuffer SSQL = new StringBuffer();//sql ËØ≠Âè•   ÊãºÊé• select    from   
        ...
        try {
            for (int i = 0; i < tableLength; i++) {
                ...
                for (int j = 0; j < iRecordCount; j++) {
                    ...
                    String SITEMCODE = getBillDataToString(record, "SITEMCODE");//Ëã±Êñá‰ª£Á†Å
                    String SITEMNAME = getBillDataToString(record, "SITEMNAME");//‰∏≠ÊñáÂêçÁß∞
                    SFIELDS.append(",");
    ...
    public String getFlowStartNodeId(HttpServletRequest request, String _flowId, String _flowVersion) {
		...
		String I_NODE_ID = "";
		String userRole = request.getSession().getAttribute("SYS_STRROLECODE").toString();// ”√ªßΩ«…´
		String userOrg = request.getSession().getAttribute("SYS_STRBRANCHID").toString(); // ”√ªß◊È÷Ø
		...
		try {
			...
			for (int i = 0; i < recordIndex; i++) { // ∂‡∏ˆø™ ºΩ⁄µ„Ω¯––≈–∂œ
				record = tableEx.getRecord(i);
				S_AUDIT_ROLE = record.getFieldByName("S_AUDIT_ROLE").value.toString(); // Ω«…´
				S_AUDIT_SQRY = record.getFieldByName("S_AUDIT_SQRY").value.toString(); // »˝»®¡¶Ãıº˛
				S_AUDIT_SQRYATTR = record.getFieldByName("S_AUDIT_SQRYATTR").value.toString();// »˝»®¡¶Ãıº˛
				I_NODE_ID = record.getFieldByName("I_NODE_ID").value.toString();
				if ("".equals(S_AUDIT_ROLE)&&!"".equals(S_AUDIT_SQRY)) { // Ω«…´”≈œ»,∆‰¥ŒŒ™∞¥≈•
